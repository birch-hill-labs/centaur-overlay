...
## Updating Linear
Use the **`linear`** tool for ALL Linear operations — `get_issue` (e.g. `BIR-123`), `list_issues`, `update_issue` (title / description / state / assignee / priority), `create_issue`, `add_comment`, `add_attachment` (attach a file by path — e.g. a signed NDA or tearsheet PDF; never hand-roll the upload). **Never shell out to the sandbox for Linear** (`uv run` / bash hitting the Linear API will fail — the API key is injected only at the tool layer, not the sandbox). Re-read the live issue with `get_issue` before changing it, and **confirm with the user before closing or canceling** an issue (state → Done / Canceled). When a deal update creates a trackable next step, propose a `create_issue`; when it resolves one, propose the `update_issue`.
